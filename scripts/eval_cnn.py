...
Usage:
    ...
    python scripts/eval_cnn.py --include-test --run-dir results/cnn/resnet18_64px_seed43
...
from scripts.train_cnn import NPZDataset, _load_64px_data, build_resnet18
from src.data.pathmnist import LABEL_NAMES

ALL_LABELS = [LABEL_NAMES[i] for i in range(len(LABEL_NAMES))]
N_CLASSES = 9
DEFAULT_RUN_DIR = PROJECT_ROOT / "results" / "cnn" / "resnet18_64px"
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Directory holding best_model.pt, and where predictions are written. "
             "Defaults to results/cnn/resnet18_64px. Set this when evaluating an "
